...
ga_map_data <- data_readin(acs_vars = "DP03_0119PE,DP04_0003PE,DP03_0009PE", acs_yr = 2019, mort_year = 2020)
glimpse(ga_map_data)
```
...
Hmisc::rcorr(as.matrix(st_drop_geometry(ga_map_data[,c("dist_to_usroad", "dist_to_treatment", "pct_poverty", "vacancy_rate", "unemployment_rate")])))

print("Spearman Correlation=================")
Hmisc::rcorr(as.matrix(st_drop_geometry(ga_map_data[,c("dist_to_usroad", "dist_to_treatment","pct_poverty", "vacancy_rate", "unemployment_rate")])),type = 'spearman')
...
ga_map_data %>% st_drop_geometry() %>% 
  pivot_longer(cols = c(pct_poverty, vacancy_rate, unemployment_rate, rucc_code13_n, dist_to_usroad, dist_to_treatment)) %>% 
...
ga_map_data %>% st_drop_geometry() %>% 
  ...
  theme_bw() +
  geom_smooth(colour = "black", span = .5, method = "lm", mapping = aes(colour = rucc_code13)) +
  labs(x = "Percent", y = "Opioid Mortality Rate", 
       title = "Plot of Indicator Association with Incidence",
       subtitle = "for GA Counties") +
  theme(guide = NULL) +
